...
public class Driver extends JPanel implements ActionListener, KeyListener {
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		bymax = 25 + y_b;
		bymin = y_b - 25;
		int ballymax = y - radius;
		int ballymin = y + radius;
		int ballxmax = x - radius;
		int ballxmin = x + radius;
		

		if(ballxmin <= bxmax && ballymin <= bymax && ballymax >= bymin){
			//if(xvelocity < 0){
			//xvelocity = -xvelocity;
			//}
			//else
				xvelocity = -xvelocity + (Math.abs(byvelocity*0.3));
				yvelocity += byvelocity*0.2;
		...
		if(x >= bx2min && y <= by2max && y >= by2min){
			//if(xvelocity < 0){
			//xvelocity = -xvelocity;
			//}
			//else
				xvelocity = -xvelocity + (Math.abs(by2velocity*0.3));
				yvelocity += by2velocity*0.2;
				System.out.println("x" + x + "y" + y);
		}
